...
const STATUS_LABEL = {
  open: "DEFINING OBJECTIVE",
  accepted: "GATHERING EVIDENCE",
  submitted: "ACCEPTANCE READY",
  in_review: "IN VALIDATION",
  blocked: "GATE FAIL",
  closed: "VALIDATED & ACCEPTED",
};
...
function renderTopbar() {
  ...
  const sMap = {
    idle:       { c: "muted",  t: "·",     l: "OFFLINE"     },
    connecting: { c: "amber",  t: "···",   l: "CONNECTING"  },
    live:       { c: "pass",   t: "●",     l: "LIVE"        },
    error:      { c: "red",    t: "⚠",     l: "ERR"         },
    mock:       { c: "muted",  t: "○",     l: "MOCK"        },
  };
  const s = sMap[ld.status] || sMap.mock;
  const fetchedAgo = ld.lastFetchedAt ? relTime(ld.lastFetchedAt.toISOString()) : "—";
  const tip = ld.lastError
    ? `Connection failed: ${ld.lastError}\nEndpoint: ${ld.cfg && ld.cfg.endpoint}`
  ...
  const shortcuts = [
    ...
    { l: "AUDIO",     h: host + "/audio-feed" },
    { l: "BRIEFING",  h: host + "/briefing" },
    { l: "OPS",       h: host + "/" },
  ];
  ...
  return `
    ...
    <nav class="shortcuts">
      ${shortcuts.map(sc => `
        <a class="sc ${sc.active?'on':''}" href="${sc.h}" target="_blank" rel="noopener">
          <span class="sc-dot"></span>${sc.l}
        </a>
      `).join("")}
    </nav>
    <div class="spacer"></div>
    <div class="datasrc ${s.c}" id="datasrc" title="${tip.replace(/"/g,'&quot;')}">
      <span class="ds-dot">${s.t}</span>
      <span class="ds-l">${s.l}</span>
      <span class="ds-sub">${ld.status==='live'?fetchedAgo+' ago':(ld.cfg && ld.cfg.mode==='live'?'reconnecting':'mock data')}</span>
      <button class="ds-refresh" title="Refresh now">↻</button>
    </div>
  ...
  return `
    ...
    <div class="rail-section">
      ...
      <ul class="nav-list">
        ...
        <li class="nav-item ${App.activeVariation==='reviewer-log'?'active':''}" data-view="reviewer-log">
          <span class="dot"></span>REVIEWER LOG
        </li>
        <li class="nav-item ${App.activeVariation==='pattern-detector'?'active':''}" data-view="pattern-detector">
          <span class="dot"></span>PATTERN DETECTOR
        </li>
        <li class="nav-item ${App.activeVariation==='meeting-room'?'active':''}" data-view="meeting-room">
          <span class="dot"></span>MEETING ROOM
        </li>
      </ul>
    </div>
  ...
  return `
    ...
    <div class="right">
      <div class="seg" id="view-seg">
        ...
        <button class="${App.activeVariation==='reviewer-log'?'on':''}" data-view="reviewer-log">REVIEWER LOG</button>
        <button class="${App.activeVariation==='pattern-detector'?'on':''}" data-view="pattern-detector">PATTERNS</button>
        <button class="${App.activeVariation==='meeting-room'?'on':''}" data-view="meeting-room">MEETING</button>
      </div>
    </div>
  `;
}
...
function ticket(j) {
  ...
  return `
    <div class="ticket ${j.priority.toLowerCase()}" data-status="${j.status}" draggable="true" data-id="${j.id}">
      <div class="row">
        <span class="pbadge ${j.priority.toLowerCase()}">${j.priority}</span>
        <span class="sbadge sbadge-${j.status}">
          <span class="sbadge-dot"></span>${STATUS_LABEL_SHORT[j.status]}
        </span>
