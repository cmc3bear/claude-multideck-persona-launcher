...
function renderBoardView(root) {
  ...
  root.innerHTML = columns.map(s => `
    <div class="col" data-status="${s}">
      <div class="col-head">
        <div><span class="swatch" style="display:inline-block"></span>${STATUS_LABEL[s]}</div>
...
function renderRadarView(root) {
  ...
  // Age scale: map 0..72h time-in-phase to 20%..95% of a column's width.
  const HOUR = 3600e3;
  const MAX_AGE = 72 * HOUR;
  const ageToWidth = (ageMs) => {
    const n = Math.max(0, Math.min(1, ageMs / MAX_AGE));
    return colPct * (0.2 + n * 0.75); // 20% min, 95% max within a column
  };
  ...
  const laneGutter = agents.map(k => {
    ...
    return `<div class="lane-row ${on?'active':''}" style="--agent-color:${p.color}" data-assignee="${k}">
      ...
      <div class="meta">
        ...
        <div class="load">${activeCount} ACTIVE <span class="loadbar"><span style="width:${loadPct}%"></span></span></div>
      </div>
    </div>`;
  }).join("");
  ...
  const laneTracks = agents.map(k => {
    ...
    const bars = agentJobs.map(j => {
      ...
      // stack offset per column
      const colKey = isBlocked ? "blocked" : PHASES[col].key;
      const row = stackByCol[colKey] || 0;
      stackByCol[colKey] = row + 1;
      const top = 6 + row * 14; // vertical stack within lane
      if (top > 44) return ""; // cap overflow

      return `<div class="radar-bar ${j.priority.toLowerCase()} ${j.status} ${stressClass}" data-id="${j.id}"
        style="left:${left}%; width:${width}%; top:${top}px; height:12px; padding:0 6px; font-size:9.5px">
        <span class="pid">#${j.id}</span>${j.subject}
        <span style="margin-left:6px;color:var(--ink-mute);font-size:9px">${ageLabel}</span>
      </div>`;
    }).join("");
...
function renderClusterView(root) {
  ...
  jobs.forEach(j => {
    ...
    el.style.left = (pt.x - 90) + "px";
    el.style.top = (pt.y - 26) + "px";
      ...
      ${topBlockers.length === 0 ? '<div class="dep-empty">— NO ACTIVE BLOCKERS —</div>' : topBlockers.map(r => {
        ...
        return `<div class="blocker-row" data-id="${r.j.id}" style="cursor:pointer">
          ...
          <div class="br-foot">
            ...
            <span>${STATUS_LABEL[r.j.status]}</span>
          </div>
        </div>`;
      }).join("")}
    </div>
